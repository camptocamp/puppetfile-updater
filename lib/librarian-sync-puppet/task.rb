...
class LibrarianSyncPuppet
    ...
    def define(args, &task_block)
      ...
      task @name do
        ...
        Augeas.open(basedir, lens_dir, Augeas::NO_MODL_AUTOLOAD) do |aug|
          ...
          aug.load!
          update_github_refs(aug, github, @user, @module)
          update_forge_refs(aug, @user, @module)
          aug.save!
        end
      end
    end
  ...
  def update_github_refs(aug, github, user='.*', mod)
  ...
  def update_forge_refs(aug, user='.*', mod)
